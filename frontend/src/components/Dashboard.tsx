...
const Dashboard: React.FC<DashboardProps> = ({ userId, onLogout }) => {
  ...
  const fetchTransactions = async () => {
    if (account) {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/transactions/${account.id}`);
  ...
  const handleNewTransaction = async (amount: number, type: string, note: string) => {
    if (account) {
      try {
        await axios.post(`${process.env.REACT_APP_API_BASE_URL}/transaction`, {
